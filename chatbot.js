// --- JRN AI CHATBOT (SINGLE-FILE SOLUTION) ---

const API_KEY = "AIzaSyAOl4tZO1DTM2LUBOFrbr-wjls6Mf84vXg"; 

// ১. HTML UI তৈরি করা
const chatbotHTML = `
<div id="chat-bubble" style="position: fixed; bottom: 25px; right: 25px; background: #38bdf8; width: 60px; height: 60px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; box-shadow: 0 10px 30px rgba(0,0,0,0.5); z-index: 10000; transition: 0.3s;">
...
<div id="chat-window" style="display: none; position: fixed; bottom: 100px; right: 25px; width: 350px; height: 500px; background: #1e293b; border: 1px solid #334155; border-radius: 20px; flex-direction: column; overflow: hidden; z-index: 10000; box-shadow: 0 15px 50px rgba(0,0,0,0.6); font-family: 'Poppins', sans-serif;">
    <div style="background: #38bdf8; padding: 15px; color: #0f172a; font-weight: 800; display: flex; justify-content: space-between; align-items: center;">
        ...
        <i class="fas fa-times" id="close-chat" style="cursor: pointer; font-size: 20px;"></i>
    </div>
    ...
    <div style="padding: 15px; background: #1e293b; border-top: 1px solid #334155; display: flex; gap: 10px;">
        <input type="text" id="user-input" placeholder="আপনার বার্তা লিখুন..." style="flex: 1; background: #0f172a; border: 1px solid #334155; color: white; padding: 12px; border-radius: 12px; outline: none; font-size: 14px;">
...
async function askGemini(message) {
    ...
    const systemPrompt = "You are JRN Assistant for 'JRN STUDIO' by Jony Roy. JRN Studio offers Web Design, Development, and AI Prompt services. Keep answers short, friendly, and use Bengali.";
...
async function handleChat() {
    ...
    chatLogs.innerHTML += `<div style="background: #38bdf8; color: #0f172a; padding: 12px 16px; border-radius: 15px 15px 0 15px; align-self: flex-end; max-width: 85%; font-size: 14px; font-weight: 600;">${text}</div>`;
    userInput.value = "";
    chatLogs.scrollTop = chatLogs.scrollHeight;

    const loading = document.createElement('div');
    loading.innerHTML = `<span style="color: #94a3b8; font-size: 12px; margin-left: 10px;">JRN AI ভাবছে...</span>`;
    chatLogs.appendChild(loading);

    try {
        const reply = await askGemini(text);
        chatLogs.removeChild(loading);
        chatLogs.innerHTML += `<div style="background: #1e293b; color: #f8fafc; padding: 12px 16px; border-radius: 15px 15px 15px 0; align-self: flex-start; max-width: 85%; font-size: 14px; border: 1px solid #334155;">${reply}</div>`;
    } catch (e) {
        chatLogs.removeChild(loading);
        chatLogs.innerHTML += `<div style="color: #ef4444; font-size: 12px;">Error connecting.</div>`;
    }
    chatLogs.scrollTop = chatLogs.scrollHeight;
}
